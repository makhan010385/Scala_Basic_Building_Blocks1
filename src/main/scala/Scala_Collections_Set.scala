// Set is used to store unique elements in the set.
// It does not maintain any order for storing elements. You can apply various operations on them.
// It is defined in the Scala.collection.immutable package.
object Scala_Collections_Set {
  def main(args: Array[String]): Unit = {
//    val set1 = Set()                            // An empty set
//    val Mtech_roll_number =Set(1,12,14,16,20)
    val Mtech_Stu_Names = Set(1,1.2,"Somesh","Kapil","Brijneder","Ritika","Tosseb")    // Creating a set with elements
//    val Mtech_Stu_Names1:Set[String] =Set("Somesh","Kapil","Brijneder","Ritika","Tosseb")
//    println(set1)  //  to Create Empty Set
//    println("First way to Defined Set:="+Mtech_Stu_Names)
// println(s"Second way to Defined Set:=$Mtech_Stu_Names")
//    println(Mtech_Stu_Names)
//    var r:Int=2
//
//    print(raw"my name \n is :=$r%f")
println(Mtech_Stu_Names.head)             // Returns first element present in the set
println(Mtech_Stu_Names.tail)         // Returns all elements except first element.
println(Mtech_Stu_Names.isEmpty)          // Returns either true or false

  }
}
